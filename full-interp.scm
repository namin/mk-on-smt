...
(define (eval-expo expr env val)
  (conde
    ...
    ((fresh (rator x* rands a* prim-id)
       (== `(,rator . ,rands) expr)
       (eval-expo rator env (make-prim prim-id))
...
(define initial-env `((list . (val . ,(make-closure 'x 'x empty-env)))
                      (not . (val . ,(make-prim 'not)))
                      (equal? . (val . ,(make-prim 'equal?)))
                      (symbol? . (val . ,(make-prim 'symbol?)))
                      (cons . (val . ,(make-prim 'cons)))
                      (null? . (val . ,(make-prim 'null?)))
                      (car . (val . ,(make-prim 'car)))
                      (cdr . (val . ,(make-prim 'cdr)))
                      . ,empty-env))
